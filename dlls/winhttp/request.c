...
DWORD WINAPI WinHttpWebSocketSend( HINTERNET hsocket, WINHTTP_WEB_SOCKET_BUFFER_TYPE type, void *buf, DWORD len )
{
    ...
    if (socket->request->connect->hdr.flags & WINHTTP_FLAG_ASYNC)
    {
        ...
        else
        {
            ...
            ret = socket_send( socket, type, buf, len, &s->ovr );
        }
...
static DWORD receive_bytes( struct socket *socket, char *buf, DWORD len, DWORD *ret_len, BOOL read_full_buffer )
{
    ...
    received = 0;
    while (size + received != len)
    {
        if ((err = netconn_recv( socket->request->netconn, ptr, needed, 0, &recv_size ))) return err;
        if (!recv_size) break;
        received += recv_size;
    ...
    *ret_len = size + received;
    if (read_full_buffer && *ret_len != len) return ERROR_WINHTTP_INVALID_SERVER_RESPONSE;
    return ERROR_SUCCESS;
}
