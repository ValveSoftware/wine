...
static struct channel_block *performance_get_channel_block(struct performance *This, DWORD block_num)
{
    ...
    else if ((block = malloc(sizeof(*block))))
    {
        block->block_num = block_num;
        wine_rb_put(&This->channel_blocks, &block->block_num, &block->entry);
    }
        ...
        MUSIC_TIME music_time, REFERENCE_TIME *time)
{
    ...
    *time = 0;
    ...
    for (tempo = 120., duration = tempo_time = 0; music_time > 0; tempo_time += next)
    {
        ...
        duration += (600000000. * next) / (tempo * DMUS_PPQ);
        music_time -= next;
    }

    duration += (600000000. * music_time) / (tempo * DMUS_PPQ);
        ...
        REFERENCE_TIME time, MUSIC_TIME *music_time)
{
    ...
    *music_time = 0;
    ...
    for (tempo = 120., tempo_time = 0; duration > 0; tempo_time += next, duration -= step)
    {
        if (FAILED(hr = IDirectMusicPerformance_GetParam(iface, &GUID_TempoParam, -1, DMUS_SEG_ALLTRACKS,
                tempo_time, &next, &param)))
            break;
        ...
        if (param.mtTime <= 0) tempo = param.dblTempo;
        step = (600000000. * next) / (tempo * DMUS_PPQ);
        if (!next || duration < step) break;
        *music_time = *music_time + next;
    }

    *music_time = *music_time + round((duration * tempo * DMUS_PPQ) / 600000000.);
...
{
    IDirectMusicPerformance8 *iface = &perf->IDirectMusicPerformance8_iface;
    IDirectMusicPort *port;
    GUID guid;
    unsigned int i;
    HRESULT hr;
    ...
    {
        if (FAILED(hr = IDirectMusicPerformance8_AssignPChannelBlock(iface, i, port, i + 1)))
            ERR("Failed to set performance channel block info, hr %#lx\n", hr);
    }
        ...
        DWORD block_num, IDirectMusicPort *port, DWORD midi_group)
{
    ...
    if (!(block = performance_get_channel_block(This, block_num))) return E_OUTOFMEMORY;
        ...
        IDirectMusicPort *port, DWORD midi_group, DWORD midi_channel)
{
    ...
    if (!(channel = performance_get_channel(This, channel_num))) return DMUS_E_NOT_FOUND;
    channel->midi_group = midi_group;
