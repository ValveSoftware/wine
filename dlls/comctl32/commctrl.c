...
LRESULT WINAPI DefSubclassProc (HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   if (!stack->stackpos) {
      ...
      WNDPROC proc;
      /* original proc may have changed, we only need its original unicode/ansi nature */
      if (stack->is_unicode)
         proc = (WNDPROC)SetWindowLongPtrW (hWnd, GWLP_WNDPROC, (DWORD_PTR)stack->origproc);
      else
         proc = (WNDPROC)SetWindowLongPtrA (hWnd, GWLP_WNDPROC, (DWORD_PTR)stack->origproc);
      ret = CallWindowProcW (stack->origproc, hWnd, uMsg, wParam, lParam);
      /* restore previous proc and original unicode/ansi nature */
      if (unicode)
         SetWindowLongPtrW (hWnd, GWLP_WNDPROC, (DWORD_PTR)proc);
      else
         SetWindowLongPtrW (hWnd, GWLP_WNDPROC, (DWORD_PTR)proc);
   } else {
