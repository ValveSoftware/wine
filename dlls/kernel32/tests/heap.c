...
static void test_heap_size( SIZE_T initial_size )
{
    ...
    get_valloc_info( heap, &current_base, &alloc_size, &commit_size );
    ...
    ok( alloc_size == initial_size + default_heap_size || broken( (initial_size && alloc_size == initial_size)
        ...
        "got %#Ix.\n", alloc_size );
    ok( commit_size == alloc_size, "got %#Ix.\n", commit_size );
    ...
    for (i = 0; i < 100; ++i)
    {
        ...
        get_valloc_info( p, &base, &alloc_size, &commit_size );
        ok( alloc_size == commit_size, "got alloc_size %#Ix, commit_size %#Ix.\n", alloc_size, commit_size );
