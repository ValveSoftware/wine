...
static void test_D3DXLoadSurface(IDirect3DDevice9 *device)
{
    ...
    else
    {
        ...
        hr = IDirect3DSurface9_LockRect(surf, &lockrect, NULL, D3DLOCK_READONLY);
        ok(hr == D3D_OK, "Failed to lock surface, hr %#lx.\n", hr);
        check_pixel_float4(&lockrect, 0, 0,  0.0f,       0.181996f,  0.363992f, 0.0f,      TRUE);
        check_pixel_float4(&lockrect, 1, 0,  0.545988f,  0.727984f,  1.0f,      0.333333f, TRUE);
        check_pixel_float4(&lockrect, 0, 1, -1.0f,      -1.0f,      -0.547945f, 0.666667f, TRUE);
        check_pixel_float4(&lockrect, 1, 1, -0.365949f, -0.183953f, -0.001957f, 1.0f,      TRUE);
        hr = IDirect3DSurface9_UnlockRect(surf);
        ok(hr == D3D_OK, "Failed to unlock surface, hr %#lx.\n", hr);
