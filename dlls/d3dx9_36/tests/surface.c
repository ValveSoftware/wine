...
static void test_D3DXLoadSurface(IDirect3DDevice9 *device)
{
    ...
    else
    {
        ...
        hr = IDirect3DSurface9_LockRect(surf, &lockrect, NULL, D3DLOCK_READONLY);
        ok(hr == D3D_OK, "Failed to lock surface, hr %#lx.\n", hr);
        check_pixel_float4(&lockrect, 0, 0,  0.0f,       0.377953f, 1.0f, 1.0f, TRUE);
        check_pixel_float4(&lockrect, 1, 0,  0.503937f,  1.0f,      1.0f, 1.0f, TRUE);
        check_pixel_float4(&lockrect, 0, 1, -1.0f,      -1.0f,      1.0f, 1.0f, TRUE);
