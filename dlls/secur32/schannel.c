...
BOOL schan_imp_create_session(schan_imp_session *session, schan_credentials *cred)
{
    ...
    return schan_imp_create_session_gnutls30(session, cred);
}

void schan_imp_dispose_session(schan_imp_session session)
{
    if (have_gnutls26) return schan_imp_dispose_session_gnutls26(session);
    return schan_imp_dispose_session_gnutls30(session);
}

void schan_imp_set_session_transport(schan_imp_session session, struct schan_transport *t)
{
    if (have_gnutls26) return schan_imp_set_session_transport_gnutls26(session, t);
    return schan_imp_set_session_transport_gnutls30(session, t);
}

void schan_imp_set_session_target(schan_imp_session session, const char *target)
{
    if (have_gnutls26) return schan_imp_set_session_target_gnutls26(session, target);
    return schan_imp_set_session_target_gnutls30(session, target);
}

SECURITY_STATUS schan_imp_handshake(schan_imp_session session)
{
    if (have_gnutls26) return schan_imp_handshake_gnutls26(session);
    return schan_imp_handshake_gnutls30(session);
}

unsigned int schan_imp_get_session_cipher_block_size(schan_imp_session session)
{
    if (have_gnutls26) return schan_imp_get_session_cipher_block_size_gnutls26(session);
    return schan_imp_get_session_cipher_block_size_gnutls30(session);
}

unsigned int schan_imp_get_max_message_size(schan_imp_session session)
{
    if (have_gnutls26) return schan_imp_get_max_message_size_gnutls26(session);
    return schan_imp_get_max_message_size_gnutls30(session);
}

ALG_ID schan_imp_get_key_signature_algorithm(schan_imp_session session)
{
    if (have_gnutls26) return schan_imp_get_key_signature_algorithm_gnutls26(session);
    return schan_imp_get_key_signature_algorithm_gnutls30(session);
}

SECURITY_STATUS schan_imp_get_connection_info(schan_imp_session session,
                                              SecPkgContext_ConnectionInfo *info)
{
    if (have_gnutls26) return schan_imp_get_connection_info_gnutls26(session, info);
    return schan_imp_get_connection_info_gnutls30(session, info);
}

SECURITY_STATUS schan_imp_get_session_peer_certificate(schan_imp_session session, HCERTSTORE store,
                                                       PCCERT_CONTEXT *cert)
{
    if (have_gnutls26) return schan_imp_get_session_peer_certificate_gnutls26(session, store, cert);
    return schan_imp_get_session_peer_certificate_gnutls30(session, store, cert);
}

SECURITY_STATUS schan_imp_send(schan_imp_session session, const void *buffer, SIZE_T *length)
{
    if (have_gnutls26) return schan_imp_send_gnutls26(session, buffer, length);
    return schan_imp_send_gnutls30(session, buffer, length);
}

SECURITY_STATUS schan_imp_recv(schan_imp_session session, void *buffer, SIZE_T *length)
{
    if (have_gnutls26) return schan_imp_recv_gnutls26(session, buffer, length);
    return schan_imp_recv_gnutls30(session, buffer, length);
}

BOOL schan_imp_allocate_certificate_credentials(schan_credentials *creds)
{
    if (have_gnutls26) return schan_imp_allocate_certificate_credentials_gnutls26(creds);
    return schan_imp_allocate_certificate_credentials_gnutls30(creds);
}

void schan_imp_free_certificate_credentials(schan_credentials *creds)
{
    if (have_gnutls26) return schan_imp_free_certificate_credentials_gnutls26(creds);
    return schan_imp_free_certificate_credentials_gnutls30(creds);
}
