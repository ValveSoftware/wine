...
static HRESULT asf_reader_init_stream(struct strmbase_filter *iface)
{
    ...
    hr = IWMReaderAdvanced_SetUserProvidedClock(reader_advanced, !filter->filter.clock);
    if (FAILED(hr))
        ...
        DWORD output, DWORD size, INSSBuffer **out, void *context)
{
    ...
    *out = NULL;
    ...
    if (!stream->source.pin.peer || !stream->source.pAllocator
            || filter->status != WMT_STARTED)
        hr = VFW_E_NOT_CONNECTED;
    else
    {
        IMemAllocator_AddRef((allocator = stream->source.pAllocator));
        hr = S_OK;
    }
