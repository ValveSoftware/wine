...
static HRESULT asf_reader_init_stream(struct strmbase_filter *iface)
{
    ...
    for (i = 0; i < filter->stream_count; ++i)
    {
        ...
        hr = IWMReaderAdvanced_SetAllocateForOutput(reader_advanced, i, TRUE);
        if (FAILED(hr))
        {
        ...
        hr = IPin_NewSegment(stream->source.pin.peer, stream->seek.llCurrent, stream->seek.llStop, stream->seek.dRate);
        if (FAILED(hr))
        {
        ...
        DWORD output, DWORD size, INSSBuffer **out, void *context)
{
    ...
    *out = NULL;
    ...
    if (!stream->source.pin.peer)
        ...
        hr = IMemAllocator_GetBuffer(stream->source.pAllocator, &sample, NULL, NULL, AM_GBF_NOWAIT);
    ...
    {
        ERR("Allocated media sample is too small, size %lu.\n", size);
        IMediaSample_Release(sample);
