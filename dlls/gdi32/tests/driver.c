...
static void test_gpu_device_properties(void)
{
    ...
    set = SetupDiGetClassDevsW(&GUID_DEVINTERFACE_DISPLAY_ADAPTER, NULL, NULL, DIGCF_DEVICEINTERFACE | DIGCF_PRESENT);
    ...
    i = 0;
    while (SetupDiEnumDeviceInterfaces(set, NULL, &GUID_DEVINTERFACE_DISPLAY_ADAPTER, i, &iface))
    {
        ret = SetupDiGetDeviceInterfaceDetailW(set, &iface, iface_data,
                sizeof(iface_detail_buffer), NULL, &device_data );
        ...
        if (!wcsicmp(device_id, L"root\\basicdisplay"))
        {
            ok(!ret, "Found Bus Id.\n");
        }
        else
