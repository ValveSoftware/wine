...
static HRESULT STDMETHODCALLTYPE api_information_statics_IsMethodPresentWithArity(
        ...
        UINT32 parameter_count, BOOLEAN *value)
{
    FIXME("iface %p, type_name %s, method_name %s, parameter_count %u, value %p stub!\n", iface,
            debugstr_hstring(type_name), debugstr_hstring(method_name), parameter_count, value);
    return E_NOTIMPL;
}
