        ...
        INSSBuffer **ret_sample, QWORD *pts, QWORD *duration, DWORD *flags, WORD *ret_stream_number)
{
    ...
    for (;;)
    {
        ...
        if (wg_buffer.discontinuity)
            flags_ex |= WM_SFEX_DATALOSS;
        if (wg_buffer.delta)
            flags_ex |= WM_SFEX_NOTASYNCPOINT;

        if (!stream->read_compressed && stream->output_allocator)
            hr = IWMReaderAllocatorEx_AllocateForOutputEx(stream->output_allocator, stream->index, wg_buffer.size,
                    &sample, flags_ex, wg_buffer.pts, wg_buffer.duration, NULL);
        else if (stream->read_compressed && stream->stream_allocator)
            hr = IWMReaderAllocatorEx_AllocateForStreamEx(stream->stream_allocator, stream->index + 1, wg_buffer.size,
                    &sample, flags_ex, wg_buffer.pts, wg_buffer.duration, NULL);
