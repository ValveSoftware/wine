...
static DWORD CALLBACK read_thread(void *arg)
{
    ...
    while (!reader->read_thread_shutdown)
    {
        ...
        if (!size)
        {
            wg_parser_push_data(reader->wg_parser, WG_READ_SUCCESS, data, 0, true);
            continue;
        }
        ...
        if (file)
        {
                    ...
                    || !ReadFile(file, data, size, &ret_size, NULL))
            {
                ...
                wg_parser_push_data(reader->wg_parser, WG_READ_FAILURE, NULL, 0, true);
                continue;
            }
        }
        else
        ...
        wg_parser_push_data(reader->wg_parser, WG_READ_SUCCESS, data, ret_size, true);
    }
...
static HRESULT init_stream(struct wm_reader *reader, QWORD file_size)
{
    ...
    if (!(wg_parser = wg_parser_create(WG_PARSER_DECODEBIN, true, false)))
        return E_OUTOFMEMORY;
