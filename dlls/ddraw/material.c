...
static ULONG WINAPI d3d_material3_Release(IDirect3DMaterial3 *iface)
{
    ...
    if (!ref)
    {
        ...
        {
            wined3d_mutex_lock();
            ddraw_free_handle(&material->ddraw->handle_table, material->Handle - 1, DDRAW_HANDLE_MATERIAL);
        ...
        IDirect3DDevice3 *device, D3DMATERIALHANDLE *handle)
{
    ...
    if (!material->Handle)
    {
        DWORD h = ddraw_allocate_handle(&device_impl->ddraw->handle_table, material, DDRAW_HANDLE_MATERIAL);
